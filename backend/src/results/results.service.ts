...
export class ResultsService {
     ...
     */
    public async startAndProcessResults(session: string, dataFilePath: string,
                                        dataFileSeparator: string, graphFilePath: string,
                                        options: { variables: string, selectedMethods: string }): Promise<void> {
        // clear previous results
        await this.redisService.del(`results:${session}`);
        loggerInstance.log('info',
            `starting estimation for ${dataFilePath} variables: ${options.variables}, methods: ${options.selectedMethods}`);
        const cwd = process.cwd();
        const proc = spawn('python3', [`${cwd}/../dowhy/process_and_return.py`,
            dataFilePath, dataFileSeparator, graphFilePath, options.variables, options.selectedMethods, session]);
